...
static void send_packet(unsigned char *data, int block_no)
{
    ...
    btHead[0] = SOH;
#else
    btHead[0] = (block_no==0)?SOH:STX;
#endif

    btHead[1] = block_no & 0xFF;
    btHead[2] = ~block_no & 0xFF;
    _putchars(btHead, sizeof(btHead));
    _putchars(data, packet_size);

    crc = __cpu_to_be16(crc);
    _putchars((unsigned char *)(&crc), sizeof(crc));
}
...
static long send_data_packets(unsigned char* data, unsigned long size)
{
	...
	int ch;
#ifdef _ALWAY_128
    int blockno_all = (size + PACKET_SIZE-1)/PACKET_SIZE;
#else
    int blockno_all = (size + PACKET_1K_SIZE-1)/PACKET_1K_SIZE;
#endif
...
#endif
        //fprintf(stdout, "send blockno %d\n", blockno);
		send_packet(data, blockno);
		ch = _getchar(PACKET_TIMEOUT);
        fprintf(stdout, "\r%d%%", (int)(blockno*1.0/blockno_all*100));
        fflush(stdout);
...
long ymodem_send(unsigned char* buf, unsigned long size, char* filename)
{
    unsigned char btCAN[2] = {CAN, CAN};
	int ch, crc_nak = 1;
    int timeout_count = 1;

	fprintf(stdout,"Ymodem send:\n");
	...
	do {
        fprintf(stdout,"Ymodem wait CRC...\n");
		ch = _getchar(1);
	} while (ch < 0 && timeout_count++ < 4);
	...
	if (ch == CRC) {
		do {
			...
			if (ch == ACK) {
				ch = _getchar(PACKET_TIMEOUT);
				if (ch == CRC) {
					if (send_data_packets(buf, size) != -1)
                    {
                        fprintf(stdout,"\nsent:%s success\n", filename);
